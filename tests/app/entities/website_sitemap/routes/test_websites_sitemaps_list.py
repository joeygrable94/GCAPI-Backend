...
async def test_list_all_website_sitemaps_as_superuser(
    ...
    entry_1 = await create_random_website_map(db_session)
    entry_2 = await create_random_website_map(db_session)
    response: Response = await client.get("sitemaps/", headers=admin_user.token_headers)
...
async def test_list_website_sitemaps_as_superuser_by_website_id(
    ...
    entry_1 = await create_random_website_map(db_session, website_id=website_a.id)
    entry_2 = await create_random_website_map(db_session, website_id=website_a.id)
    entry_3 = await create_random_website_map(db_session, website_id=website_a.id)
    entry_4 = await create_random_website_map(db_session, website_id=website_b.id)
    entry_5 = await create_random_website_map(db_session, website_id=website_b.id)
...
async def test_list_all_website_sitemaps_as_employee(
    ...
    response: Response = await client.get(
        "sitemaps/", headers=employee_user.token_headers
    )
    assert 200 <= response.status_code < 300
    data: Any = response.json()
    assert data["page"] == 1

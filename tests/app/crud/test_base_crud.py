...
async def test_client_encrypted_too_in_db(db_session: AsyncSession) -> None:
    ...
    a_client = await repo.read(client.id)
    assert a_client.title == client.title


async def test_db_base_query_list(db_session: AsyncSession) -> None:
    repo: ClientRepository = ClientRepository(session=db_session)
    client_list_select = repo.query_list(is_active=True)
    assert isinstance(client_list_select, Select)

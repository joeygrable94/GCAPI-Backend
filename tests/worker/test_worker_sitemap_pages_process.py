...
async def test_task_website_sitemap_process_pages() -> None:
    ...
    with unittest.mock.patch("app.worker.save_sitemap_pages") as mock_save_sitemap_pages:
        mock_save_sitemap_pages.return_value = get_uuid()

        sitemap_id = await task_website_sitemap_process_pages(website_id, sitemap_url, sitemap_pages)

        assert isinstance(sitemap_id, UUID)

        mock_save_sitemap_pages.assert_called_once_with(website_id, sitemap_url, sitemap_pages)

...
async def test_create_or_update_website_page_create() -> None:
    ...
    output = await create_or_update_website_page(website_id, sitemap_id, page)
...
async def test_create_or_update_website_page_create_then_update() -> None:
    ...
    sitemap_id: UUID4 = get_uuid()
    page = SitemapPage(url=page_url, priority=0.5)
    output_a = await create_or_update_website_page(website_id, sitemap_id, page)
    output_b = await create_or_update_website_page(website_id, sitemap_id, page)

...
async def test_create_website_pagespeedinsights(db_session: AsyncSession) -> None:
    ...
    website_map = await create_random_website_map(db_session, website.id)
    website_page = await create_random_website_page(
        db_session, website.id, website_map.id
    )
...
async def test_create_website_pagespeedinsights_website_not_found(
    ...
    website_id = get_uuid()
    website_map = await create_random_website_map(db_session, website_id)
    website_page = await create_random_website_page(
        db_session, website_id, website_map.id
    )
    d_strategy: str = "mobile"
    psi_base: WebsitePageSpeedInsightsBase = generate_psi_base(
        device_strategy=d_strategy

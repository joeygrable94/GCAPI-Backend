...
async def test_list_all_ga4_property_as_superuser(
    ...
    response: Response = await client.get("ga4/property/", headers=admin_token_headers)
...
async def test_list_all_ga4_property_as_employee(
    ...
    a_client_b_website = await assign_website_to_client(  # noqa: F841
        db_session, b_website, a_client
    )
    ...
    response: Response = await client.get(
        "ga4/property/", headers=employee_token_headers
    )
    assert 200 <= response.status_code < 300
    data: Any = response.json()
    assert data["page"] == 1

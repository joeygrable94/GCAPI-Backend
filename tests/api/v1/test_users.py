...
async def test_update_current_superuser(
    client: AsyncClient,
    current_superuser: Tuple[UserRead, Dict[str, str]],
) -> None:
    ...
    assert current_user.is_verified == True
...
async def test_list_users_as_superuser(
    ...
    user_1: UserRead = await create_random_user(user_auth)
    user_2: UserRead = await create_random_user(user_auth)
...
async def test_list_users_as_testuser(
    ...
    user_1: UserRead = await create_random_user(user_auth)  # noqa: F841
    user_2: UserRead = await create_random_user(user_auth)  # noqa: F841
    response: Response = await client.get("users/", headers=testuser_token_headers)
    error: Dict[str, Any] = response.json()
    assert response.status_code == 401
    assert error["detail"] == ErrorCode.USER_FORBIDDEN
...
async def test_get_user_by_id_as_superuser(
    ...
    user: UserRead = await create_random_user(user_auth)
    response: Response = await client.get(
        f"users/{user.id}",
        headers=superuser_token_headers,
    )
...
async def test_update_user_as_random_user_forbidden(
    ...
    b_user: UserRead = await create_random_user(user_auth)
    update_dict = UserUpdate(
        password="NEWvalidPassw0rd",
        email=b_user.email,
    )

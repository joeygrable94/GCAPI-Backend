...
class BaseRepository(
    ...
    async def _list(
        ...
        data: List[TABLE] = result.scalars().all()
        return data

    async def list(self, page: int = 1) -> Optional[Union[List[TABLE], List[None]]]:
    ...
    async def create(self, schema: Union[SCHEMA_CREATE, Any]) -> TABLE:
        entry: Any = self._table(id=self.gen_uuid(), **schema.dict())
        self._db.add(entry)

...
def configure_routers(app):
    ...
    app.include_router(fastapi_users.get_reset_password_router(), prefix=f"{settings.API_PREFIX}/auth", tags=["auth"])
    app.include_router(fastapi_users.get_verify_router(UserRead), prefix=f"{settings.API_PREFIX}/auth", tags=["auth"])
    # user routes

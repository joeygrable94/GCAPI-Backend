...
Permission = configure_permissions(get_active_user_privileges)
...
- **access controll lists**: a list of rules defining which *privilege* has what *permission*
- **privilege**: an identifier of a user or his/her associated groups/roles
...
2. a privilege: e.g. "role:admin" or "user:bob"
...
The two privileges ```Everyone``` and ```Authenticated``` will be discussed in short time.

### users & privilege identifiers

You **must provide** a function that returns the privileges of the current active user. The privileges is just a list of strings, identifying the user and groups/roles the user belongs to:

Example:

```python
def get_active_privileges(user: User = Depends(get_current_user)):
    if user:
        # user is logged in
        privileges = [Everyone, Authenticated]
        privileges.extend(getattr(user, "privileges", []))
    else:
...
There are two special privileges that also help providing access controll lists: ```Everyone``` and ```Authenticated```.

The ```Everyone``` privilege should be added regardless of any other defined privileges or login status, ```Authenticated``` should only be added for a user that is logged in.

### permissions

A permission is just a string that represents an action to be performed on a resource. Just make something up.

As with the special privileges, there is a special permission that is usable as a wildcard: ```fastapi_permisssions.All```.
...
- a callable ([FastApi dependency][dependency]) that returns the privilege of the logged in (active) user
...
def get_active_privileges(...):
    """ returns the privileges of the current logged in user"""
    ...

# Permission is already wrapped in Depends()
Permission = configure_permissions(get_active_privileges)
```
...
With ```has_permission(user_privileges, permission, resource)``` you can preform the permission check programatically. The function signature can easily be remebered with something like "John eat apple?". The result will be either ```True``` or ```False```, so no need for try/except blocks \o/.
...
user_privileges == [Everyone, Authenticated, "role:owner", "user:bob"]
apple_acl == [(Allow, "role:owner", All)]

if has_permission(user_privileges, "eat", apple_acl):
    print "Yum!"
```

The other function provided is ```list_permissions(user_privileges, resource)``` this will return a dict of all available permissions and a boolean value if the permission is granted or denied:
...
print(list_permissions(user_privileges, apple_acl))
{"permissions:*": True}
```

...
from app.core.config import PROJ_KEY, settings


def generate_api_keys(file_key: str = PROJ_KEY) -> None:
...
def load_api_keys(file_key: str = PROJ_KEY) -> Tuple[rsa.PrivateKey, rsa.PublicKey]:
    pub_pem_file = f"{settings.ROOT_DIR}/{file_key}_private_key.pem"
    pvt_pem_file = f"{settings.ROOT_DIR}/{file_key}_public_key.pem"
    if not os.path.exists(pub_pem_file) and not os.path.exists(pvt_pem_file):
        generate_api_keys(PROJ_KEY)
        load_api_keys(PROJ_KEY)

...
class Logger:
	def __init__(self, name='myapp', level='INFO'):
		self.file_frmt = logging.Formatter('%(levelname)s:     %(message)s [%(asctime)s]')
		self.stream_frmt = logging.Formatter('%(levelname)s:     %(message)s [%(asctime)s]')
	...
	def log(self, *args):
		self.logger.info(default_theme.style_normal+''.join(str(i) for i in args)+default_theme.style_normal)

	def debug(self, *args):
		self.logger.debug(default_theme.style_prompt+''.join(str(i) for i in args)+default_theme.style_normal)

	def info(self, *args):
		self.logger.info(default_theme.style_normal+''.join(str(i) for i in args)+default_theme.style_normal)

	def exception(self, *args):
		self.logger.exception(default_theme.style_fail+''.join(str(i) for i in args)+default_theme.style_normal)
